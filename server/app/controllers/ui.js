...
const handlers = {
  ...
  deleteCongregation: async function (req, res) {
    const congregation = await DAL.deleteCongregation(req.params.congregationId);
    return res.redirect('/ui/congregations');
  },
  ...
  updateLanguage: async function (req, res) {
    const synonyms = (req.payload.synonyms || '')
      ...
      .split('\n')
      .filter(x => x && x.length);
    await DAL.updateLanguage({ languageId: req.params.languageId }, {
      synonyms,
      language: req.payload.language,
    });
  ...
  addCongregationIntegration: async function (req, res) {
    const sourceCongregationId = Number(req.params.sourceCongregationId || req.payload.sourceCongregationId);
    const destinationCongregationId = Number(req.params.destinationCongregationId || req.payload.destinationCongregationId);
    const congregationId = req.params.congregationId || req.payload.destinationCongregationId;
  ...
  deleteCongregationIntegration: async function (req, res) {
    const sourceCongregationId = req.params.sourceCongregationId || req.payload.sourceCongregationId;
    const destinationCongregationId = req.params.destinationCongregationId || req.payload.destinationCongregationId;
    const congregationId = req.params.congregationId || (req.payload && req.payload.congregationId) || req.query.congregationid;
