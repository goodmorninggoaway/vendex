...
exports.handler = async function findTerritory({
  ...
  const containingTerritories = await DAL.findTerritoryContainingPoint(
    congregationId,
    location,
  );
  const originalTerritoryId = destinationCongregationLocation
    ? destinationCongregationLocation.territoryId
    : null;
  let originalTerritory;
  if (originalTerritoryId) {
    originalTerritory = await DAL.findTerritory({
  ...
  if (!containingTerritories.length) {
    if (originalTerritoryId) {
      return {
        ...
        $message:
          'Could not find a matching territory, but this location is assigned to a territory. You may need to import the latest territories.',
      ...
      return {
        territory: originalTerritory,
        $message:
          'Could not find a matching territory. You may need to import the latest territories.',
  ...
  if (containingTerritories.length > 1) {
    ...
    return {
      territory: originalTerritory,
      $message: `This location falls within the boundaries of ${
        containingTerritories.length
      } territories: ${idList}`,
      $messageLevel: MESSAGE_LEVEL.CONFLICT,
    };
  }
